...
- Check if the object's material name is contained in the _dynamic material database_. If so, assign the material.
- Check if the object's material name is contained in the _static material database_. If so, assign the material.
...
The [`features/ecommerce/`](https://github.com/shapediver/AppBuilderShared/tree/development/features/ecommerce/) feature provides integration with e-commerce APIs. It defines a typed [e-commerce API interface](https://github.com/shapediver/AppBuilderShared/tree/development/features/ecommerce/config/ecommerceapi.ts) that can be connected to any e-commerce backend. App Builder widgets can trigger actions such as adding items to a cart ([AppBuilderActionAddToCartComponent](https://github.com/shapediver/AppBuilderShared/tree/development/widgets/appbuilder/ui/AppBuilderActionAddToCartComponent.tsx)) or closing a configurator ([AppBuilderActionCloseConfiguratorComponent](https://github.com/shapediver/AppBuilderShared/tree/development/widgets/appbuilder/ui/AppBuilderActionCloseConfiguratorComponent.tsx)).

An e-commerce plugin (Shopify, WooCommerce, or similar) that embeds App Builder in an iframe should import [`ECommerceApiFactory`](https://github.com/shapediver/AppBuilderShared/tree/development/features/ecommerce/api/ecommerceapi.ts) and the types from [`config/ecommerceapi.ts`](https://github.com/shapediver/AppBuilderShared/tree/development/features/ecommerce/config/ecommerceapi.ts). That surface depends only on `zod`, `post-robot`, and ShapeDiver SDKs — not React or Mantine. Typical setup: `ECommerceApiFactory.getConnectorApi(iframe.contentWindow, actions, "plugin", "app", options)`. Do not import [`features/ecommerce/model/`](https://github.com/shapediver/AppBuilderShared/tree/development/features/ecommerce/model/) (App Builder React hooks). Add AppBuilderShared as a git submodule and set both `@AppBuilderShared` and `@AppBuilderLib` path aliases to that directory (see the [AppBuilderShared README](https://github.com/shapediver/AppBuilderShared)).

#### Stargate integration (Desktop client integration)

The [`entities/stargate/`](https://github.com/shapediver/AppBuilderShared/tree/development/entities/stargate/) entity enables integration with [ShapeDiver Stargate](https://help.shapediver.com/doc/shapediver-desktop-clients), which allows the browser-based App Builder to communicate with a locally running desktop application. This unlocks use cases such as bi-directional parameter exchange and output transfer between the web app and a CAD desktop environment. Dedicated widgets are available for this integration: [AppBuilderDesktopClientOutputsWidgetComponent](https://github.com/shapediver/AppBuilderShared/tree/development/widgets/appbuilder/ui/AppBuilderDesktopClientOutputsWidgetComponent.tsx) and [AppBuilderDesktopClientSelectionWidgetComponent](https://github.com/shapediver/AppBuilderShared/tree/development/widgets/appbuilder/ui/AppBuilderDesktopClientSelectionWidgetComponent.tsx).
