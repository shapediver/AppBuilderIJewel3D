...
interface IWebGiStoreViewport {
	...
	closeViewport: (
		viewportId: string,
		callbacks?: IWebGiStoreViewportCallbacks,
	) => Promise<void>;
	createViewport: (
		dto: ViewportCreateDto,
		callbacks?: IWebGiStoreViewportCallbacks,
	) => Promise<CoreViewerApp | undefined>;
 ...
 */
export const useWebGiStoreViewport = create<IWebGiStoreViewport>()(
	devtools(
		(set, get) => ({
			viewports: {},
			...
			createViewport: async (dto: ViewportCreateDto, callbacks) => {
				...
				if (
					Object.keys(viewports).findIndex((v) => identifier === v) >=
					0
				)
					return;
